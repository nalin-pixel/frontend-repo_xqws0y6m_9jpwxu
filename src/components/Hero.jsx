...
const Hero = () => {
  return (
    <section className="relative min-h-[100vh] w-full overflow-hidden bg-slate-950 text-white">
      ...
      {/* Readability & Color Overlays (non-blocking) */}
      <div className="pointer-events-none absolute inset-0 bg-gradient-to-b from-fuchsia-600/20 via-purple-700/10 to-slate-950/80" />
      <div className="pointer-events-none absolute -top-40 left-1/2 h-[60rem] w-[60rem] -translate-x-1/2 rounded-full bg-[radial-gradient(circle_at_center,rgba(244,114,182,0.18),transparent_60%)] blur-2xl" />
      ...
      <div className="relative z-10 mx-auto flex min-h-[80vh] max-w-6xl flex-col items-center justify-center px-6 text-center">
        ...
        <motion.h1
          ...
          className="mb-3 bg-gradient-to-r from-pink-300 via-fuchsia-400 to-purple-500 bg-clip-text text-4xl font-extrabold tracking-tight text-transparent sm:text-6xl"
        >
          Full‑Stack Developer
        </motion.h1>
        <motion.p initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.9, delay: 0.05 }} className="mx-auto max-w-2xl text-base text-slate-100/90 sm:text-lg">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.9, delay: 0.15 }} className="mt-8 flex flex-wrap items-center justify-center gap-4">
          <a href="#about" className="group relative rounded-full px-7 py-3 text-sm font-medium">
            <span className="absolute inset-0 rounded-full bg-gradient-to-r from-pink-500 to-purple-600 opacity-90 transition group-hover:opacity-100" />
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.8, duration: 1 }} className="absolute bottom-6 left-1/2 hidden -translate-x-1/2 items-center gap-2 text-xs text-white/70 sm:flex">
